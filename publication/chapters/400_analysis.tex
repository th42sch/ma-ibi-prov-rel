...
progressing digital transformation. In this study, we adopt their
...
Furthermore, \gls{FRBR} contains relations of two kinds, i.e.,
relations linking Group-1 entities with each other,
and relations linking Group-1 entities with the Group-2 entity \term{ResponsibleEntity}.
The relations of the first kind are \term{realization}, \term{embodiment}, \term{exemplar},
and their subordinate relations \term{relatedEndeavour} and \term{part},
as well as the converses of all these relations (e.g., \term{realizationOf}, \term{partOf}).
The relations of the second kind are \term{creator}, \term{realizer}, \term{producer}, \term{owner},
and their subordinate relation \term{responsibleEntity},
as well as their converses (e.g., \term{creatorOf}).
These entities and relations are shown in Figure~\ref{fig:FRBR}.
...
\begin{figure}[ht]
  \centering
  \alt{FRBR entities and relations as described in the text above. Entities of Group 1 and 2 are on the left and right, respectively. Relations of the first kind are on the left; those of the second kind are between left and right.}
  \begin{tikzpicture}[
    >=Latex,
...
them to each other: Bibliographic records (\enquote{\foreignlanguage{ngerman}{Titeldatensätze}})
...
containing the relevant description, and record the source of the information 
...
The \gls{OAI} is a project devoted to interoperability standards in information agencies, including libraries.
The \gls{OAI-PMH} is a standard developed by the OAI
and widely used in the library domain, albeit not as widely as \gls{Z39.50}.
...
As of 31 December 2022, K10plus contains 80.8 million bibliographic records (\enquote{\foreignlanguage{ngerman}{Titeldatensätze}})
...
\footnote{Personal communication with \foreignlanguage{ngerman}{Jakob Voß}, VZG (head office of the \gls{GBV}).}
...
Annotations of statements (see above) seem suitable for 
recording metadata provenance and temporal attributes of relationships.
It remains to study the use of annotations systematically
and determine the extent to which metadata provenance and temporal attributes
are recorded.
...
the \gls{SoNAR} project and Hakelberg's work \parencite*{Hakelberg2016}.
For the abstract model to be developed next,
the specific choice of data sources is largely unimportant
because the model should be independent of the contents or specific data models of the sources.
The same holds for the general query answering method that we are going to develop based on our model.
