...
in \exaquery{2} can be instantiated with the seminal work \emph{\foreignlanguage{latin}{De revolutionibus orbium coelestium}}
...
For example, the online catalogue (OPAC) of the Gotha Research Library of the University of \foreignlanguage{ngerman}{Erfurt (Forschungsbibliothek Gotha)} lists two printed exemplars
...
%}
One of those bears the signature \sig{\foreignlanguage{ngerman}{Druck}~4°~00466}, and its provenance entries name the following previous owners \autocite{OPACDeRevPPN}; see also Figure~\ref{fig:OPAC_derev_provenance}:
%\footnote{%
...
\begin{figure}[ht]
  \centering
  \alt{Part of an OPAC search result showing 8 provenance entries, including autographs, a handwritten note, and stamps. These entries are described in the preceding text.}
  ...
  \caption[%
    ...
    exemplar \sig{\foreignlanguage{ngerman}{Druck}~4°~00466} of
    ...
    for exemplar \sig{\foreignlanguage{ngerman}{Druck}~4°~00466} of
...
contains a reference to \foreignlanguage{ngerman}{Thau} via the relation \term{student}, see Figure~\ref{fig:GND_Hommel}
...
%  \fbox{\includegraphics[width=.7\linewidth,trim=140 210 150 104,clip]{img/gnd_hommel_1.png}}
  \alt{Part of the GND entry for Johann Hommel, containing the individual's GND ID, the preferred name, years of birth and death, professions (mathematician, astronomer), and a reference to his student Valentin Thau.}
...
Unfortunately, the data available does not imply that \foreignlanguage{ngerman}{Hommel} passed the exemplar (directly) to \foreignlanguage{ngerman}{Thau};
...
\enquote{\sig{\foreignlanguage{ngerman}{Druck}~4°~00466}}, if we assume that our researcher interprets the data retrieved generously
...
\begin{enumerate}
  \item
    ...
    \footnote{%
      ...
      reveals that \gls{GND} does have an entry for the subject term \term{\foreignlanguage{ngerman}{Wissenschaftler}} (scientist) \autocite{GNDScientist},
      but this term is used only sporadically: its 46 subordinate terms do not include, for example,
      \term{\foreignlanguage{ngerman}{Mathematiker}}, \term{\foreignlanguage{ngerman}{Astronom}},
      or \term{\foreignlanguage{ngerman}{Theologe}} (mathematician, astronomer, theologian)  \autocite{GNDScientistSub}.%
    }
    ...
    If the query uses such missing terms, then the answer set is empty
...
%    \todo[think,inline]{Explain this further? A case for ontologies!}
  \item
    ...
    \emph{passed it on} to another owner. The same problem affects Query Pattern~\exaquery{4}.
    Analogously,
    ...
    or who \emph{knows} another person.
