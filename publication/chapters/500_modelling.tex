...
consists of pairs of objects including $(\term{Graph\_Theory},\term{\foreignlanguage{ngerman}{Reinhard\_Diestel}})$
...
For example, the converse \term{creator\_of} of the relation \term{creator}
...
e.g., \term{\foreignlanguage{ngerman}{Reinhard\_Diestel}} is an instance of \term{Person},
...
\begin{wrapfigure}[6]{o}{5.2cm}
  ...
  \alt{A directed graph consisting of 5 nodes and 8 edges.}
  \begin{tikzpicture}[
    >=Latex,
...
labelled with both \term{student} and \term{collaborator}
...
\newcommand{\tikzexagraphMinusCopernicus}[3][]{%
  \node [text width=\derevnode,#1]                                (work1)   {\fns\mystrut\term{\foreignlanguage{latin}{De\_revolutionibus}}};
  \node [text width=\derevnode,below=25mm of work1]               (item1)   {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{FB\_Gotha\_}}}{\term{\foreignlanguage{ngerman}{Druck}\_4°\_00466}}};
  \node [text width=\copernode,above right=11mm and 45mm of item1] (person2) {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{Johann}\_}}{\term{\foreignlanguage{ngerman}{Hommel}}}};
  \node [text width=\copernode,below right=11mm and 45mm of item1] (person3) {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{Valentin}\_}}{\term{\foreignlanguage{ngerman}{Thau}}}};
...
%      (person3) edge[bend right=10] node[pos=.54,right=1mm]    {\fns\tikztabtwo{\term{is\_student\_of,}}{\term{collaborates\_with}}} (person2)
      ($(work1.270)+(-0.08,0)$)   edge node[pos=#2,left=.5mm]          {\fns\term{exemplar}}      ($(item1.90)+(-0.08,0)$)
      ($(item1.90)+(0.08,0)$)     edge node[pos=#3,right=.5mm]         {\fns\term{exemplarOf}}    ($(work1.270)+(0.08,0)$)
      
      ($(item1.0)+(0,0.4)$)      edge node[pos=.5,sloped, above=.7mm] {\fns\term{owner}}   ($(person2.180)+(0,0.08)$)
      ($(person2.180)+(0,-0.08)$) edge node[pos=.5,sloped, below=.5mm] {\fns\term{ownerOf}} ($(item1.0)+(0,0.24)$)

      ($(item1.0)+(0,-0.24)$)     edge node[pos=.5,sloped, above=.7mm] {\fns\term{owner}}   ($(person3.180)+(0,0.08)$)
      ($(person3.180)+(0,-0.08)$) edge node[pos=.5,sloped, below=.5mm] {\fns\term{ownerOf}} ($(item1.0)+(0,-0.4)$)
      
      ($(person2.270)+(-0.08,0)$) edge node[pos=.46,left=.5mm]         {\fns\tikztabtwo[r]{\term{student,}}{\term{collaborator}}} ($(person3.90)+(-0.08,0)$)
      ($(person3.90)+(0.08,0)$)   edge node[pos=.54,right=.5mm]        {\fns\tikztabtwo{\term{studentOf,}}{\term{collaborator}}}  ($(person2.270)+(0.08,0)$)
    ;
      
    \node[above=.7mm of work1]   () {\fns\term{Work}};
    \node[below=.7mm of item1]   () {\fns\term{Item}};
    \node[right=.7mm of person2] () {\fns\tikztabthree{\term{Person,}}{\term{Scientist,}}{\term{Mathematician}}};
    \node[right=.7mm of person3] () {\fns\tikztabthree{\term{Person,}}{\term{Scientist,}}{\term{Astronomer}}};
...
\begin{figure}[ht]
  \centering
  \alt{A directed graph with 5 nodes representing the objects and individuals described in the text above and their corresponding labels, plus the respective edges labelled with "exemplar", "creator", "owner", "student", "collaborator" (and their inverses).}
  \begin{tikzpicture}[
    >=Latex,
  ...
  \caption{A labelled directed graph that represents data
    concerning an exemplar of Copernicus' \emph{\foreignlanguage{latin}{De revolutionibus}} and some of its owners}
...
\begin{itemize}
  \item
    The node at the top left has the name \term{\foreignlanguage{latin}{De\_revolutionibus}}
    and the single label \term{Work}.
  \item 
    The node at the bottom right has the two labels \term{Person} and \term{Astronomer}.
  \item 
    The edge from the node named \term{\foreignlanguage{ngerman}{Johann\_Hommel}} to the node named \term{\foreignlanguage{ngerman}{Valentin\_Thau}}
...
\newcommand{\tikzexaquery}[1][1]{%
  \begin{scope}[%
    ...
    \node                                        (derev) {\fns\mystrut$\term{\foreignlanguage{latin}{De\_revolutionibus}}$};
    \node [ansvar,below=17mm of derev]           (x)     {\fns\mystrut$x$};
    \node [anovar,above right=6.0mm and 20mm of x] (y)     {\fns\mystrut$y$};
    \node [anovar,below right=6.0mm and 20mm of x] (z)     {\fns\mystrut$z$};
  \end{scope}
...
%      (y)     edge node[right=1mm]          {\fns\tikztabtwo[r]{\term{has\_}}{\term{student}}} (z)      
      (derev) edge node[left=.7mm]           {\fns\term{exemplar}} (x)
      (x)     edge node[sloped, above=.7mm] {\fns\term{owner}}         (y)
      (x)     edge node[sloped, below=.5mm]      {\fns\strut\term{owner}}   (z)
      (y)     edge[shorten >= -1.5pt, shorten <= -1.5pt] node[right=1mm,pos=.4] {\fns\term{student}} (z)      
    ;
      
    \node[above=.9mm of derev] () {\fns\term{Work}};
    \node[left=.5mm of x]      () {\fns\term{Item}};
    \node[above=.3mm of y]     () {\fns\term{Scientist}};
    
  \end{scope}
}
%
\begin{figure}[ht]
%\begin{wrapfigure}[10]{o}{5.4cm}
  \centering
  \alt{The rectangle-shaped node "De revolutionibus" is labelled "Work" and has an "exemplar" edge to a round node x labelled "Item". Node x has two "owner" edges to diamond-shaped nodes y and z, where y is labelled "Scientist" and has a "student" edge to z.}
  \begin{tikzpicture}[
    >=Latex,
  ...
  \caption{Graph representation of Query \exaquery{2$'$}}
  \label{fig:graph_for_exa_query2'}
%\end{wrapfigure}
\end{figure}

To model this query, we do not only need a node representing the work \emph{\foreignlanguage{latin}{De revolutionibus}},
...
\begin{enumerate}[(1)]
  \item
    Node \tikzinlinenode{\term{\foreignlanguage{latin}{De\_revolutionibus}}} represents that work;
  \item
...
the required node named \term{\foreignlanguage{latin}{De\_revolutionibus}}.
...
\begin{figure}[ht]
  ...
  \begin{tabular}{@{}c@{\hspace{10mm}}c@{}}
    (a) & (b) \\[-6pt]
    \alt{Node "De revolutionibus" is labelled "Work" and has an "exemplar" edge to node "FB Gotha Druck 4° 00466", which is labelled "Item" and has 2 "owner" edges to nodes "Hommel", "Thau". Node "Hommel" is labelled "Scientist" and has a "student" edge to "Thau".}
    \begin{tikzpicture}[
      >=Latex,baseline=0pt,
      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm,align=center},
      every edge/.style={draw=black,thick}
    ]
      \node [text width=\derevnode]                     (derev) {\fns\mystrut$\term{\foreignlanguage{latin}{De\_revolutionibus}}$};
      \node [text width=\derevnode,below=14mm of work1] (x)     {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{FB\_Gotha}\_}}{\term{\foreignlanguage{ngerman}{Druck}\_4°\_00466}}};
      \node [above right=7.5mm and 37mm of x] (y)     {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{Johann}\_}}{\term{\foreignlanguage{ngerman}{Hommel}}}};
      \node [below right=7.5mm and 37mm of x] (z)     {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{Valentin}\_}}{\term{\foreignlanguage{ngerman}{Thau}}}};
...
%          (y)     edge node[right=1mm]          {\fns\tikztabtwo[r]{\term{has\_}}{\term{student}}} (z)      
    ...
    \end{tikzpicture}
    &
    \alt{The graph from the previous subfigure, plus an additional converse "exemplarOf" edge, plus additional labels "Person" for node "Hommel" and "Person", "Scientist" for "Thau".}
    \begin{tikzpicture}[
      >=Latex,baseline=0pt,
      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm,align=center},
      every edge/.style={draw=black,thick}
...
%          (y)     edge node[right=1mm]          {\fns\tikztabtwo[r]{\term{has\_}}{\term{student}}} (z)      
        ...
        \node[above=.5mm of derev] () {\fns\term{Work}};
        \node[below=.5mm of x]     () {\fns\term{Item}};
        \node[above=.5mm of y]     () {\fns\tikztabtwo{\term{Person}}{\term{Scientist}}};
        \node[below=.5mm of z]     () {\fns\tikztabtwo{\term{Person}}{\term{Scientist}}};
      \end{scope}
    \end{tikzpicture}
    \\[6pt]
    (c) & (d)
    \\[-6pt]
    \alt{The graph from the first subfigure, minus the "student" edge from "Hommel" to "Thau".}
    \begin{tikzpicture}[
      >=Latex,baseline=0pt,
      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm,align=center},
      every edge/.style={draw=black,thick}
...
%          (y)     edge node[right=1mm]          {\fns\term{student}} (z)      
    ...
    \end{tikzpicture}
    &
    \alt{The graph from the first subfigure, but with the first two nodes named "ABC" and "DEF".}
    \begin{tikzpicture}[
      >=Latex,baseline=0pt,
      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm,align=center},
      every edge/.style={draw=black,thick}
      ...
      \node [below=14mm of work1]             (x)     {\fns\mystrut$\term{DEF}$};
      \node [above right=7.5mm and 37mm of x] (y)     {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{Johann}\_}}{\term{\foreignlanguage{ngerman}{Hommel}}}};
      \node [below right=7.5mm and 37mm of x] (z)     {\fns\tikztabtwo{\term{\foreignlanguage{ngerman}{Valentin}\_}}{\term{\foreignlanguage{ngerman}{Thau}}}};
...
from the previous query to the previous graph.
...
\begin{figure}[ht]
  \centering
  \alt{The previous query graph on the left, the data graph from Figure 5.2 on the right, and four edges labelled h: from "De Revolutionibus" left to "De Revolutionibus" right, and from x, y, z to "FB Gotha Druck 4° 00466", "Hommel", "Thau", respectively.}
  \begin{tikzpicture}[
    >=Latex,
    every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm,align=center},
    every edge/.style={draw=black,thick}
    ...
    \tikzexaquery
    \tikzexagraph[right=68mm of derev]{.28}{.72}
    \tikzhmph      
  \end{tikzpicture}
  %  
  \caption{A homomorphism from \exaquery{2$'$} to the graph from Figure~\ref{fig:example_graph}}
...
\begin{figure}[ht]
  \centering
  \alt{The same as the previous figure, minus the node "Copernicus" and its incoming/outgoing edges. The query graph is slightly greyed out, putting the focus on the h edges and the remaining data graph, which together constitute the answer.}
  \begin{tikzpicture}[
    >=Latex,
    every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm,align=center},
    every edge/.style={draw=black,thick}
    ...
    \tikzexaquery[.5]
    \tikzexagraphMinusCopernicus[right=68mm of derev]{.28}{.72}
    \tikzhmph
  \end{tikzpicture}
  %
...
So far, we have built a model that captures data sources and query patterns such as \exaquery{2}.
...
The basic model that we have so far developed captures not just Query Pattern~\exaquery{2}
...
\end{enumerate}
%
In the case of \exaquery{1}, the relation \term{reads} is unlikely to be recorded in provenance entries
in any biblio\-graphic data source. Therefore, we need to resort to the \emph{substitute information}
(see Section~\ref{sec:HNA+SoNAR}) provided by the \term{owner} relation.
Now Figure~\ref{fig:modelling_Q145} shows how to model these query patterns.

\begin{figure}[ht]
  \centering
  \alt{4 graphs depcting the above query patterns. E.g., the graph for Q1- has nodes X, y1, x1, y2, x2 labelled Work, Expression, Manifestation, Item, Person and connected, in this order, with a chain of edges labelled realization, embodiment, exemplar, owner.}
  \begin{tikzpicture}[
    >=Latex,baseline=0pt,
    every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm},
    every edge/.style={draw=black,thick}
  ...
  \caption{Graph representations of Query Patterns \exaquery{1$^-$},\exaquery{4}, \exaquery{5$^-$}, and \exaquery{8}}
  \label{fig:modelling_Q145}
\end{figure}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{Relations with Attributes and Relations of Higher Arity}
\label{subsec:attributes+n-ary}

The full wording of \exaquery{1} and~\exaquery{5} includes references to years
as attributes of relationships.
In the case of \exaquery{5}, we restrict ourselves to a variant that names
...
\end{enumerate}
%
In~\exaquery{5$^\star$}, the year is specified as an attribute of the \term{provider} relation
while, in~\exaquery{1}, it acts as an additional answer variable that represents
an attribute of the \term{owner} relation.
In order to capture~\exaquery{1} and~\exaquery{5$^\star$} fully,
...
%for both graphs and queries, 
as the attributes can be added to
the respective edge labels. This is shown in Figure~\ref{fig:attributes_on_relships}
for the two respective edges of~\exaquery{1} and~\exaquery{5$^\star$}.

\begin{figure}[ht]
  \centering
  \begin{tabular}{@{}c@{\hspace*{20mm}}c@{}}
    \alt{A graph depicting the abobe query pattern Q1. The graph has two nodes y2, x2 labelled Item and Person, respectively, and an edge "owner : x3" from y2 to x2.}
    \begin{tikzpicture}[
      >=Latex,baseline=0pt,
      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm},
      every edge/.style={draw=black,thick}
    ...
    \end{tikzpicture}
    &
    \alt{A graph depicting the abobe query pattern Q5*. The graph has two nodes x, Y labelled Item and ResponsibleEntity, respectively, and an edge "provider : 1935" from x to Y.}
    \begin{tikzpicture}[
      >=Latex,baseline=0pt,
      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm},
      every edge/.style={draw=black,thick}
...
%\begin{figure}[ht]
\begin{wrapfigure}[13]{o}{6.8cm}
  \centering
%  \vspace*{-.5\baselineskip}
  \alt{Graph for query pattern Q3, having nodes X, y1, Y, y2, x-bar labelled Work, Expression, Manifestation, Item, Person, and connected, in this order, with edges labelled realization, embodiment, exemplar, owner. Set node x-bar has a new octagon shape.}
  \begin{tikzpicture}[
    >=Latex,baseline=0pt,
    every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm},
    every edge/.style={draw=black,thick}
    ...
    \node [anovar,below=16mm of Q3X]  (Q3y1) {\fns\mystrut$y_1$};
    \node [       below=16mm of Q3y1] (Q3Y)  {\fns\mystrut$~~~~Y~~~~$};
    \node [anovar,below=16mm of Q3Y]  (Q3y2) {\fns\mystrut$y_2$};
    \node [setvar,right=24mm of Q3y2] (Q3x)  {\fns\mystrut$\overline{x}$};
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{Further Types of Information}
...
Query Patterns~\exaquery{1}--\exaquery{9} use several relationships
...
Furthermore, \exaquery{4} and \exaquery{2$'$}
...
\paragraph{Complex relationships}
...
via two \term{student}-edges that point from the nodes representing $P_1$ and $P_2$
...
as can be seen, for example, in the full versions of Query Pattern~\exaquery{5}:
%
\begin{enumerate}
  \item[\exaquery{5}]
    Which items from the holdings of library $X$ were acquired from bookseller $Y$
...
for example, 
by replacing the edge label \enquote{\term{provider}\mybold{\,:\,1935}}
on the right-hand side of Figure~\ref{fig:attributes_on_relships}
with \enquote{\term{provider}\mybold{\,:\,$\geqslant$\,1933 \& $\leqslant$\,1945}.}

Temporal constraints can also be useful for modelling relations
that are not explicit in the data, such as \enquote{passed on to}, which occurs in~\exaquery{2} and~\exaquery{4}.
...
This can be expressed using attributes of the \term{owner} relation
and a \enquote{$<$} constraint between the attributes of the \term{owner} labels
of the edges pointing to $y_1$ and $y_2$.

In order to incorporate such constraints,
...
for example, the following variant of \exaquery{4}.
%
\begin{enumerate}
  \item[\exaquery{4$'$}]
...
\end{enumerate}
%
This example uses a disjunction of the relations \term{child} and \term{student}.
...
So far, we have not addressed Query Patterns~\exaquery{6} and~\exaquery{7} in this chapter:
%
\begin{enumerate}
  \item[\exaquery{6}]
...
The participation in a sale (\exaquery{6}) is a very general notion that needs to be specified
\emph{before} formulating the query and attempting to match it to the available data.
The paths of ownership featuring in \exaquery{7} are a complex notion that cannot be captured by a simple query;
...
We also have not yet discussed the requirements identified by the SoNAR report
and listed in Section~\ref{subsec:SoNAR_reports},
other than \SonarR{016}--\SonarR{018}.
