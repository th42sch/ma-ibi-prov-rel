...
\newcommand{\tikzexagraph}[1][]{%
  ...
  \begin{scope}[%
    ...
    \path[->]
      (work1)   edge[bend right=10] node[pos=.4,left=1mm]      {\fns\term{has\_exemplar}}         (item1)
...
\newcommand{\tikzexaquery}{%
  ...
  \begin{scope}[%
    ...
    \path[->]
      (derev) edge node[left=1mm]           {\fns\term{has\_exemplar}}      (x)
      (x)     edge node[sloped, above=.6mm] {\fns\term{has\_owner}}         (y)
      (x)     edge node[sloped, below]      {\fns\strut\term{has\_owner}}   (z)
      (y)     edge node[right=1mm]          {\fns\strut\term{has\_student}} (z)      
    ;
      
    \node[left=.5mm of derev] () {\fns\term{Work}};
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \tikzexaquery
    \tikzexagraph[right=50mm of derev]
    
    \begin{scope}[%
      every node/.style={draw=none,fill=none,inner sep=.2mm}
      every edge/.style={densely dashed,draw=black,thick}
    ]
      \path[->]
        (derev) edge (work1)
        (x)     edge (item1)
