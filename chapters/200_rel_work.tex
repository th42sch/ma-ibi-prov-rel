...
In the paper of the same name, \citeauthor*{Menzel2020} \autocite*{Menzel2020} report on
...
In the described approach, the authors integrate data from 6 heterogeneous sources
...
The original sources comprise an authority file (GND \autocite{GND}),
...
\emph{named entity linking}, is discussed further by \citeauthor*{Menzel2021} \autocite*{Menzel2021}
in the context of SoNAR,
and by \citeauthor*{Meiners2022} \autocite*{Meiners2022} independently of SoNAR.

Among the challenges associated specifically with processing data,
\citeauthor*{Menzel2020} \autocite*{Menzel2020} name
...
\todo[inline]{Mention/discuss related projects from \autocite[§2.1]{Menzel2020}}

\todo[inline]{Some more comments on NEL and choice of data sources from \autocite*{Menzel2021, Meiners2022}? see Exposé}
...
for future work. For the same reason, we will spend very little attention
on visualisation, which is an important component
of SoNAR.
...
*** CONTINUE DISCUSSION WITH INSIGHTS FROM \autocite{Menzel2020}

\todo[inline]{My motivation and delineation from HNA/SoNAR/further approaches in \autocite{Menzel2020}:}

\begin{itemize}
  \item
    \emph{local} queries, no \enquote{graph-wide} methods
  \item 
    no \enquote{complete} answering of a research questions; instead, providing of evidence, candidates for further \emph{exploration}
  \item
    \emph{in-place} answering of a query $\leadsto$ no construction of a graph required, but consultation of single repositories \enquote{on the fly} (however, works on \enquote{integrated} graph as well)
  \item
    no a-priori restriction to specific entity or relation types
  \item
    tool for exploration \emph{in addition to} visualisation; provision of \emph{entry points}
\end{itemize}
