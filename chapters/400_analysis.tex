...
the information in Subsections~\ref{subsec:data_comm_prot} and~\ref{subsec:data_descr_schemata}
...
\glsunset{Z39.50}%
\gls{Z39.50} is a set of rules developed specifically
for translating search and retrieval commands between databases such as OPACs.
Using \gls{Z39.50}, it is possible to issue commands specific to a local database
and obtain results from a remote database that uses a poossibly different command set.
This protocol can be implemented over the internet and similar networks.
\gls{Z39.50} is widely used in the library domain for providing access to catalogues,
...
\gls{SRU} is a successor of \gls{Z39.50} and \gls{LoC} standard.
It has the same function, which is the standardisation of search and retrieval
across online databases. Being based on \gls{HTTP}
and \gls{XML}, \gls{SRU} is more dynamic and more generic than \gls{Z39.50}.
...
(21, \gls{XML}, Turbomarc, MAB2 as a MARC \enquote{family member} \autocite[p.204]{Hider2008})
...
\gls{SRU} and \gls{OAI}, which support \gls{XML} serialisations of the data formats
MARC~21, \gls{RDF}, DNB Casual (an \gls{XML}-based \gls{DC} format), and MODS.\todo{introduce all these abbreviations}
