...
we need to analyse the sources missing or spurious data and their effects on the answers obtained.
...
\begin{enumerate}
  \item
    ...
    of the world.
    The same effect can occur with relations instead of students:
    ...
    More generally speaking, concepts and relations can be missing in the data
    because\todo{find proof in the lit./with experts? Ask DH?}
    membership in them is implicit, for example, from membership in more specific concepts
    or relations. If the query uses such concepts or relations, then the answer set is always empty
    ...
    most concepts and relations have several superordinate concepts or relations and,
  ...
  \item
    ...
    More generally speaking, concepts or relations can be missing in the data
    ...
    for example, relationships such as who has actually \emph{read} a book are very hard to confirm,
    or relations may not be part of the fixed vocabulary for a data field in a source.\todo{find example?}
    If the query uses such concepts or relations, then the answer set is always empty, as in Case~1.
  \item    
...
on the answer set: Cases~3 and~4 have effects on single answers only, i.e., some answers are missing spurious.
...
\todo[inline]{highlight: advantages for researchers, *candidate* answers}
