...
\begin{figure}[ht]
  \centering
  \missingfigure{draw Haus vom Nikolaus or similar}
...
\newcommand{\tikzexagraph}[1][]{%
  ...
  \node [below=21mm of work1]               (item1)   {\fns\tikztabtwo{\term{FB Gotha}}{\term{Druck~4°~00466}}};
...
\begin{itemize}
  \item
    The node at the top left has the name \term{De revolutionibus}
    and the single label \term{Work}.
  \item 
    The node at the bottom right has the two labels \term{Person} and \term{Astronomer}.
  \item 
    The edge from the node named \term{Johann Hommel} to the node named \term{Valentin Thau}
...
\end{itemize}
%
\todo[inline]{add underscore to object names in text and pictures? :-(}

%%
%\begin{equation*}
%  \Nmc(v_1) = \term{De revolutionibus}
%  \qquad
%  \Lmc(v_1) = \{\term{Work}\}
%  \qquad
