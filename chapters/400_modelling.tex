...
In order to obtain a generic approach, we aim at providing definitions
for these central concepts that are intensional rather than extensional.
In particular, our definitions should depend neither on concrete example queries or data sources
such as the ones discussed in Chapter~\ref{chap:intro},
nor on concrete objects, concepts, or relationships (such as ``Copernicus'', ``copy of a work'', or ``student'').
Instead, we will develop an abstract model that provides definitions
...
An obvious source of generic definitions would be 



As a central notion for representing data sources
and queries, we use directed graphs with labels.
The concept of a graph is standard in computer science and discrete mathematics;
see, e.g., the textbook by Diestel \cite{Diestel2012}.
Graphs and graph techniques are used to represent large knowledge bases. \todo{citation}
They provide an easy means for visualising objects and their relationships,
and they lend themselves to computationally well-behaved algorithms for query answering. \todo{citation}
...
See Figure~\ref{fig:example_graph}.\todo{not FRBR-conform, explain}
