...
\begin{itemize}
  \item
    \emph{Scope:}~
    thematic focus; coverage; number of datasets; standards for cataloguing
  \item
...
which are represented by almost 26 million datasets in the German National Bibliography.

Metadata can be obtained from the DNB freely (under the CC0 1.0 licence) via the following interfaces and machine-readable formats.
%
\begin{itemize}
  \item
    \gls{SRU}:
    
    XML serialisations of the formats MARC~21, \gls{RDF}, DNB Casual (an XML-based Dublin Core format), MODS\todo{introduce all these abbreviations}
  \item
    \gls{OAI}:
    
    the same formats as \gls{SRU}
  \item
    individual access via the \enquote{Data Shop}:
    
    MARC~21 and \gls{RDF} in various serialisations, DNB Casual, CSV
\end{itemize}
%
In particular, the DNB's Linked Data Service provides open access to its bibliographic and authority data
in RDF under the Creative Commons Zero (CC0 1.0) license.
...
in relation with each other: Datasets have a bibliographic level for describing a manifestation
...
However, it is possible to create and link an authority dataset for a work or expression
and record the relevant description there. In that case, the source of the information can be recorded as well
...
\dots
The GND is operated by the DNB \enquote{in cooperation with many other libraries, libray networks and other cultural and academic institutions.
At present, the GND contains around 9 million authority records for persons, corporate bodies, congresses, geographic entities, specialised terms and works; these are supplemented, updated and used frequently} \autocite{DNB_cataloguing}.
