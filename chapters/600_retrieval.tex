...
For example, if the a query \exaquery{2$'$} is modified to ask for a scientist who passed the item on
...
then the answer set consists of $n^2 - n$ answers because there are $n \cdot (n-1)$ pairs of owners.
...
\footnote{%
  ...
  to those of the latter is a homomorphism. The number of such mappings is $n^m$,
  where $n,m$ are the numbers of nodes in the data graph and the query graph, respectively.
...
Since we are advocating an iterative runtime phase where query answers can help users (re)formulate their queries,
