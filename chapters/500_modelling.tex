...
in the following, which consists of the standard terms from conceptual
modelling \autocite{***}.\todo{cite}
...
contains the pairs $(\term{Reinhard\_Diestel},\term{Graph\_Theory})$.
...
%  \item
%    An \emph{$n$-ary relation} is a set of $n$-tuples of objects;
%    for example, the binary relation \term{autor\_of}
...
\todo[quick,inline]{Distinguish clearly between \enquote{relation} and \enquote{relationship} in the this and the remaining chapters.}
...
\emph{to} a work,
whereas is converse \term{creator\_of} points into the opposite direction.
...
(and/or an edge from $P_2$ to $P_1$ with the label $\{\term{is\_student\_of},\term{collaborates\_with}\}$).
...
\begin{itemize}
  ...
  \item
    Discuss further SoNAR insights, e.g., substitute relationships
