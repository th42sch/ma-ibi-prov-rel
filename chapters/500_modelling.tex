...
but also \exaquery{4}, as well as \exaquery{1} and \exaquery{5} if the year(s) are left out.
%More precisely,
%if we substitute the variable $X$ in \exaquery{1} with \enquote{Les Pensées De Pope} by Alexander Pope (***--***),
%in \exaquery{4} with the private library of Prince Albert of Saxe-Coburg and Gotha (1819–1861)
%and in \exaquery{arg1}
%
%
%, the following queries, which are instances of these three patterns
%(without years) can be modelled as shown in Figure~\ref{fig:modelling_Q145}.
%%
%\begin{enumerate}
%  \item[\exaquery{1$'$}]
%    Who read \enquote{Les Pensées De Pope} by Alexander Pope,
%    and in which manifestation?
%  \item[\exaquery{4$'$}]
%    Which exemplars from the private library of Prince Albert of Saxe-Coburg and Gotha (1819–1861)
%    were passed on by Prince Albert to a family member?
%  \item[\exaquery{5$'$}]
%    Which exemplars from the holdings of library $X$ were acquired from bookseller $Y$?
%%    between 1933 and 1945?
%\end{enumerate}
That is, the following query patterns can be modelled as shown in Figure~\ref{fig:modelling_Q145}
...
In order to capture~\exaquery{1} and~\exaquery{5} fully, i.e., include the year of reading in~\exaquery{1}
or of acquisition in~\exaquery{5},
...
More formally speaking, the notion of a graph (Definition~\ref{def:ld_graph})
...
but which may also contain pairs of relation names and literals/objects (for relationships with attributes).
...
\begin{figure}[ht]
  ...
  \begin{tabular}{@{}c@{\hspace*{20mm}}c@{}}
    \begin{tikzpicture}[
      ...
      \node [ansvar,right=34mm of y2] (x2) {\fns\mystrut$x_2$};
  
      \begin{scope}[%
        every node/.style={draw=none,fill=none,inner sep=.2mm},
      ]
        \path[->]
          (y2) edge[shorten <= -1.5pt] node[above=.6mm,pos=.46] {\fns\term{owner}\mybold{:1742}} (x2)
        ;
