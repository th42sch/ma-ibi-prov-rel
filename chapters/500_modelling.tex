...
known from querying \gls{RDF} graphs via \gls{SPARQL} \autocite{DellaValle2011}
...
the specific notion of a graph (Sections~\ref{sec:labelled_digraphs},
...
or undirected. Graphs are easy to visualise; nodes are represented as circles
...
Edges represent relations, which are directed by the above definition 
(as the order of components in a pair matters).
For example, an edge representing the relation
\term{creator} points \emph{from} a person or corporate body
\emph{to} a work,
whereas is converse \term{creatorOf} points into the opposite direction.
...
For example, a node representing the physicist Albert Einstein (object)
...
with the label $\{\term{student},\term{collaboratesWith}\}$
(and/or an edge from $P_2$ to $P_1$ with the label $\{\term{studentOf},\term{collaborator}\}$).
...
%      (person3) edge[bend right=10] node[pos=.54,right=1mm]    {\fns\tikztabtwo{\term{is\_student\_of,}}{\term{collaborates\_with}}} (person2)
      (work1)   edge[bend right=10] node[pos=.3,left=1mm]      {\fns\term{exemplar}} (item1)
      (item1)   edge[bend right=10] node[pos=.7,right=1mm]     {\fns\term{exemplarOf}}      (work1)
      (work1)   edge[bend left=4]   node[pos=.5,sloped, above] {\fns\strut\term{creator}}    (person1)
      (person1) edge[bend left=4]   node[pos=.5,sloped, below] {\fns\strut\term{creatorOf}} (work1)
      (item1)   edge[bend left=14]  node[pos=.5,sloped, above] {\fns\strut\term{owner}}      (person2)
      (person2) edge[bend right=6]  node[pos=.5,sloped, below] {\fns\strut\term{ownerOf}}   (item1)
      (item1)   edge[bend right=6]  node[pos=.5,sloped, above] {\fns\strut\term{owner}}      (person3)
      (person3) edge[bend left=14]  node[pos=.5,sloped, below] {\fns\strut\term{ownerOf}}   (item1)
      (person2) edge[bend right=10] node[pos=.46,left=1mm]     {\fns\tikztabtwo[r]{\term{student,}}{\term{collaborator}}} (person3)
      (person3) edge[bend right=10] node[pos=.54,right=1mm]    {\fns\tikztabtwo{\term{studentOf,}}{\term{collaborator}}} (person2)
    ;
