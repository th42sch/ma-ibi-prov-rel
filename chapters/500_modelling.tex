...
\begin{figure}[ht]
  \centering
%  \begin{tabular}{@{}c@{\hspace*{8mm}}c@{\hspace*{4mm}}c@{\hspace*{8mm}}c@{}}
%    \begin{tikzpicture}[
%      >=Latex,baseline=0pt,
%      every node/.style={on grid,rectangle,rounded corners=1mm,draw=black,fill=lightblue,thick,inner sep=1.5mm},
%      every edge/.style={draw=black,thick}
...
%      ]
%        \path[->]
%          (X)  edge[shorten >= -1.5pt] node[right=1mm]          {\fns\term{realization}} (y1)
...
%      \node [anovar,below=16mm of x] (y) {\fns\mystrut$y$};
%      \node [right=25mm of y]        (X) {\fns\mystrut$~~~~X~~~~$};
%  
%      \begin{scope}[%
%        every node/.style={draw=none,fill=none,inner sep=.2mm},
%      ]
%        \path[->]
%          (x) edge[shorten >= -1.5pt] node[right=1mm,pos=.4]   {\fns\term{holds}} (y)
%          (y) edge[shorten <= -1.5pt] node[above=.6mm,pos=.46] {\fns\term{owner}} (X)
%        ;
%          
%        \node[right=.5mm of x] () {\fns\term{Library}};
%        \node[below=.5mm of y] () {\fns\term{Item}};
%%        \node[below=.5mm of Y] () {\fns\term{ResponsibleEntity}};
%      \end{scope}
%    \end{tikzpicture}
%    \\[4mm]
...
%  \end{tabular}
  ...
  \caption{Graphs representing example query patterns \exaquery{1$^-$},\exaquery{4}, \exaquery{5$^-$}, and \exaquery{8}}
...
our basic model needs to be extended such that 
query graphs may contain variable nodes that represent sets of objects.
For the visualisation of query graphs, a new kind of node needs to be added;
Figure~\ref{fig:modelling_Q8} depicts the graph for \exaquery{3}
using a boldface letter $x$ for the \emph{set variable} and
a light pink octagon for the corresponding \emph{set node}.
...
%  \end{tikzpicture}
  ****** TODO ******
  ...
  \caption{A graph representing example query pattern \exaquery{8}}
  \label{fig:modelling_Q8}
\end{figure}
