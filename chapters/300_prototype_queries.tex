...
\end{enumerate}
%
These patterns can be instantiated by substituting a specific object for each variable,
yielding a specific \emph{query}. For example, if we substitute for the variable $X$
in \exaquery{2} the seminal work \emph{De revolutionibus orbium coelestium}
...
(short: \emph{De revolutionibus}; English translation: \emph{On the Revolutions of the Heavenly Spheres}) \autocite{Kopernikus1543}
by the astronomer Nicolaus Copernicus (1473–1543).
We thus consider the following query.
%
\begin{enumerate}
  \item[\exaquery{2$'$}]
%    Which exemplars of \emph{De revolutionibus} were owned by scientists who passed them on to a student?
...
contains a reference to Thau via the relation \enquote{has student}
...
\todo[think,inline]{Discuss instances of \exaquery{1}, \exaquery{3}--\exaquery{9}, and their differences (extend remarks from Section~\ref{sec:background})?}
