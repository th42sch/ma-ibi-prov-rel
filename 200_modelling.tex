...
\todo[inline]{%
  Explain that we could also use database theory, that the use of graphs is essentially a restriction to unary and binary predicates,
  and that we do this in order to keep the presentation digestible for readers with little or no background in maths and CS
...
%
\begin{definition}
  Given two labelled directed graphs $G = (V,E,\Nmc,\Lmc)$ and $G' = (V',E',\Nmc',\Lmc')$
  over the same namespace $(\NI,\NC,\NR)$,
  a \emph{homomorphism from $G$ to $G'$} is a map $h : V \to V'$ that satisfies the following properties.
  %
  \begin{enumerate}
    \item[\hmph{1}]
      $\Nmc(v) = \Nmc'(h(v))$ for every node $v \in V$ with $\Nmc(v) \in \NI$. \todo{VARs?}
    \item[\hmph{2}]
